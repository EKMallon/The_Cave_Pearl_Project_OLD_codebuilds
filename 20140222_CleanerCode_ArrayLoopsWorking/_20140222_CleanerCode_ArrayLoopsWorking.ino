...
// free ram stable at around 715 with 5 cycles
...
#define BMA250              0x18
#define BW                  0x08 //7.81Hz bandwidth
#define GSEL                0x03 // set range 0x03 - 2g, 0x05 - 4, 0x08 - 8g, 0x0C - 16g

#define SampleInterval 1   // power-down time in minutes before interrupt triggers the next sample
#define SamplesPerSRAMCycle 10  //#  of sample cycles before writing to the sd card 
...
float RTCTempfloat;
char SRAMCycleTimeStamp[ ]= "0000/00/00,00:00:00";
byte Cycle=0;

const byte chipSelect = 10;  //sd card chip select

uint8_t dataArray[16]; //variables for accellerometer reading
int8_t BMAtemp;        //why does the bma temp read out as an integer? Temp is in units of 0.5 degrees C
//8 bits given in two's complement representation
float BMAtempfloat;//float BMATempHolder;
//char BMATempHolder[ ]= "00.00";
//components for holding bma temp as two integers - we have no negative temps in our application
uint8_t wholeBMAtemp[SamplesPerSRAMCycle],fracBMAtemp[SamplesPerSRAMCycle];  

int x,y,z;   //these guys range to negative values
int xAcc[SamplesPerSRAMCycle],yAcc[SamplesPerSRAMCycle],zAcc[SamplesPerSRAMCycle];

uint8_t wRTCtemp[SamplesPerSRAMCycle],fRTCtemp[SamplesPerSRAMCycle]; //components for holding RTC temp as two intergers
int temp3231;
int Vcc[SamplesPerSRAMCycle];//the supply voltage via 1.1 internal band gap
